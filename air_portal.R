library(RSelenium);library(rvest);library(dplyr)
...
} else colnames(df) <- c("항공사", "편명", "목적지", "계획", "예상", "출발", "구분", "현황")

# 날짜와 공항 칼럼 생성
df$날짜 <- rep(date, nrow(df))
df$공항 <- rep(airport_kor[which(airport_code == airport)], nrow(df))

csv_file_name <- paste0(date, "_", dep_arr, ".csv")
